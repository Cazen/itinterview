...
public class TryQuestionResource {
    ...
    @Inject
    private UploadQustionService tryQuestionService;

    /**
     * POST  /tryQuestion : Create a new tryQuestion.
     *
    ...
    public ResponseEntity<UpQuestionMaster> createUpQuestionMaster(@RequestBody UpQuestionMaster upQuestionMaster) throws URISyntaxException {
        ...
        UpQuestionMaster result = tryQuestionService.save(upQuestionMaster);

        return ResponseEntity.created(new URI("/app/tryquestion/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("upQuestionMaster", result.getId().toString()))
            .body(result);
    }

    /**
     * PUT  /tryQuestion : Updates an existing upQuestionMaster.
     *
    ...
    /**
     * GET  /tryQuestion : get all the upQuestionMasters.
     *
    ...
    public ResponseEntity<List<UpQuestionMaster>> getAllUpQuestionMasters(@ApiParam Pageable pageable)
        ...
        Page<UpQuestionMaster> page = tryQuestionService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/app/tryquestion");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }

    /**
     * GET  /tryQuestion/:id : get the "id" upQuestionMaster.
     *
    ...
    public ResponseEntity<UpQuestionMaster> getUpQuestionMaster(@PathVariable Long id) {
        ...
        UpQuestionMaster upQuestionMaster = tryQuestionService.findOne(id);
    ...
    /**
     * DELETE  /tryQuestion/:id : delete the "id" upQuestionMaster.
     *
     * @param id the id of the upQuestionMaster to delete
     * @return the ResponseEntity with status 200 (OK)
     */
    @DeleteMapping("/tryquestion/{id}")
    @Timed
