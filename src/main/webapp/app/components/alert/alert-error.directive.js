(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('itinterviewApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-itinterviewApp-error');
                var entityKey = httpResponse.headers('X-itinterviewApp-params');
                if (errorHeader) {
